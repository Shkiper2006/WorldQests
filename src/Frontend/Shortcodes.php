...
final class Shortcodes
{
    ...
    public function renderQuestViewer(array $atts = []): string
    {
        ...
        $questId = (int) $atts['id'];
        if ($questId <= 0) {
            return '<div class="world-quest-error">' . esc_html__('Quest ID is required.', 'worldquest') . '</div>';
        }

        $quest = $this->wpdb->get_row($this->wpdb->prepare("SELECT id, title, slug, status FROM {$this->questsTable} WHERE id=%d", $questId), ARRAY_A);
        if (!is_array($quest)) {
            return '<div class="world-quest-error">' . esc_html__('Quest not found.', 'worldquest') . '</div>';
        }
        ...
        if ($nodes === []) {
            return '<div class="world-quest-error">' . esc_html__('No published nodes yet.', 'worldquest') . '</div>';
        }
        ...
        wp_add_inline_script('world-quest-viewer', 'window.WorldQuestViewerData = ' . wp_json_encode([
            ...
            'nodes' => $nodes,
            'choices' => $choices,
            'ctaLabel' => __('Добавить свой вариант развития событий', 'worldquest'),
